...
def optimised_result_helper(azim, elev, fname=""):
    ...
    model = align_heliostat_field(hstats, apparent_inc_vec, exp.RECEIVER_POSITION.value, exp.MIRROR_SEPERATION.value, tilts)
    ...
    if fname != "":
        ...
        img.save(fname)
        print(f"Image saved! (a={azim}, B={elev})")

    return deltas, collect_frac

def datagen_angle_range(azimuths, elevations, worker_threads=8):
    all_deltas = {}
    collect_fracs = {}
    ...
    for i, elev in enumerate(elevations):
        ...
        for result in results:
            all_deltas[(elev, azimuths[j])] = result[0]
